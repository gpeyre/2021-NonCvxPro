function [x,cBest,cnt] = prox_l1group_rank1_becker(x0, D, L, lambda, n1,n2)
...
% Rfun = @(x) reshape(x, [], k);
% shrinkVec = @(x,d) group_thresh(Rfun(x),d(1));

% Rs = @(w) reshape(w,n1,n2);
% vec = @(w) w(:);

% shrinkVec = @(x,d) vec(group_thresh(Rs(x), d(1)));
shrinkVec = @(x,d) group_thresh(x, d(1),n1,n2);
...
%each row of x is a group
function z = group_thresh(x, t,n1,n2)
x = reshape(x,n1,n2);
no = sqrt(sum(x.^2,2));
z = (max(no-t,0)./no).*x;
z(isnan(z)) = 0;
